...
function Option({ name, icon, to }) {
    ...
    return (
        <Link
            className={cls(
                "group option flex items-center gap-2 w-full h-[30px] min-h-[30px] my-[1px] mx-0 py-5 px-2 text-[--c3-txt] text-base rounded-sm transiton hover:bg-[--c3] ",
                {
                    "bg-[--c3]": isActive,
                }
            )}
        ...
        >
            ...
            <span className="flex-1 block text-nowrap text-ellipsis overflow-hidden opacity-1 ">
                {name}
            </span>
        </Link>
    );
